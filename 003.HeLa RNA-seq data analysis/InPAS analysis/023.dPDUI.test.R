...
limma_test_out <- test_dPDUI(eset = eSet,
                       method = "limma",
                       normalize = "none",
                       design = design,
                       ...
                       robust = TRUE,
                       sqlite_db = sqlite_db)
saveRDS(limma_test_out, file = file.path(outdir, "010.limma.test.out.RDS"))
limma_filter_out <- filter_testOut(res = limma_test_out,
                             gp1 = gp1,
                             gp2 = gp2,
                             adj.P.Val_cutoff = 0.05,
                             dPDUI_cutoff = 0.2,
                             PDUI_logFC_cutoff = 0.59)
write.table(limma_filter_out, file = file.path(outdir, "010.limma.test.filtered.out.txt"),
           sep = "\t", quote = FALSE, row.names = FALSE)
