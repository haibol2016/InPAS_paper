...
library(tximport)
#setwd(r"(C:\Users\liuh\OneDrive - University Of Massachusetts Medical School\Desktop\InPAS\05.featurecount.table)")
...
counts <- txi$counts
len <- txi$length
count_per_base <- as.data.frame((counts *50)/ len)

## split into list
sample_name <- gsub("_[NT]_.+", "", colnames(count_per_base), perl = TRUE)
...
for (i in seq(1,144, 2)){
  this_count_per_base <- count_per_base[, i:(i+1)]
  this_count_per_base$total_counts <- rowSums(counts[, i:(i+1)])
  this_count_per_base$avg_length <- rowMeans(len[, i:(i+1)])
  dat <- merge(this_count_per_base, gene_tx_map_biotype, 
               by.x ="row.names",
               by.y = "tx_id", all.x = TRUE)
  rownames(dat) <- dat[, 1]
  dat <- dat[, -1]
  
  keep <- rowSums(dat[,1:2]) > 0
  dat <- dat[keep, ]
  dat$total_basecov <-  log10(rowSums(dat[,1:2]))
  ...
  high_long <- high_long[!high_long$tx_type %in% c("snoRNA",
                                                   ...
                                                   "snRNA","vaultRNA"), ]
  # 
  # pdf("Fig 3.Expression.level.of.kept.high.long.tx.by.tx.biotypes.pdf", width = 10, height = 6)
  ...
  out <- "~/arthur_mercurio/Haibo/InPAS/TCGA_KIRC_BRCA/docs/BRCA.paired.sample.tx.expressed.list"
  if (!dir.exists(out)){
    dir.create(out)
  }
  print(dim(high_long))
  write.table(rownames(high_long), 
                file = file.path(out, 
                                 paste0(sample_name[i],".expressed.tx.list.txt")),
...
## gene-level count
tx2gene <- read.delim("~/arthur_mercurio/Haibo/InPAS/docs_2/02.gencode.v34.primary_assembly.Tx.gene.mapping",
                      header = FALSE)
txi <- tximport(files, type = "salmon", txOut = FALSE,
                tx2gene = tx2gene)
txi <- txi$counts
txi <- txi[rowSums(txi) > 0, ]
write.table(txi, file = "Nine.cancer.type.gene.count.table.txt",
